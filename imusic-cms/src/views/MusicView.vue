...
import SongListAdd from "@/components/SongListAdd.vue";
...
const songAddVisible = ref(false)
...
function deleteSong(row) {
  instance.delete(`/songs/delete/${row.id}`)
      .then(res => {
        if (res.data.success === true) {
          ElMessage.success('歌曲删除成功')
        } else {
          ElMessage.success('歌曲删除失败')
        }
        getMusic()
      })
      .catch(err => {
...
<template>
  <div class="w-full h-full">
    ...
    <!-- 添加歌曲到歌单 -->
    <SongListAdd v-model:visible="songAddVisible"/>
      ...
      <el-table :data="tableData()" border style="width: 100%" max-height="450">
        ...
        <el-table-column label="操作" fixed="right" width="180">
          <template #default="scope">
            ...
            <el-button type="success" link @click="addSong(scope.row)">添到歌单</el-button>
